...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUPPORT_USERS:
        rt += "Requested Anie to promote a Support user to Sudo."
        SUPPORT_USERS.remove(user_id)

    if user_id in WHITELIST_USERS:
        rt += "Requested Anie to promote a Whitelist user to Sudo."
...
def addsupport(
    ...
    if user_id in SUDO_USERS:
        rt += "Requested Anie to demote this Sudo to Support"
    ...
    if user_id in WHITELIST_USERS:
        rt += "Requested Anie to promote this Whitelist user to Support"
        WHITELIST_USERS.remove(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUDO_USERS:
        message.reply_text("Requested Anie to demote this user to Civilian")
        SUDO_USERS.remove(user_id)
...
nations = """ Anie has bot access levels we call as *"Nation Levels"*
\n*Anie Bot* - Devs who can access the bots server and can execute, edit, modify bot code. Can also manage other Nations
\n*God* - Only one exists, bot owner.
Owner has complete bot access, including bot adminship in chats Anie is at.
\n*Royals* - Have super user access, can gban, manage Nations lower than them and are admins in Anie.
\n*Sakuras* - Have access go globally ban users across Anie.
...
\n*Disclaimer*: The Nation levels in Anie are there for troubleshooting, support, banning potential scammers.
Report abuse or ask us more on these at [Anie](https://t.me/AnieBots).
"""
...
@kigcmd(command=["devlist", "anie"])
@whitelist_plus
def devlist(update: Update, context: CallbackContext):
    bot = context.bot
    true_dev = list(set(DEV_USERS) - {OWNER_ID})
    reply = "<b>Anie Members :</b>\n"
